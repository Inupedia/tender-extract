...
SiliconFlow 已使用真实 `Qwen/Qwen3-8B` 接口完成验收：**4/4 网络调用成功**，同一文档第二次运行 **4 次全部命中缓存、0 次新增网络调用**，对应标注集的 **微平均 F1 / 宏平均 F1 均为 1.000**。外部模型耗时受网络和服务端排队影响，因此不与上面的 34.62 页/秒本地基线混在一起计算。
